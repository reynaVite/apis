...
// Endpoint para guardar datos de rezago académico
app.post('/guardar-rezago-academico', async (req, res) => {
  let connection;
  const { idAlumnos, descripcion } = req.body;

  try {
    // Establecer conexión desde el pool
    connection = await pool.getConnection();

    // Consulta SQL para insertar datos de rezago académico
    const query = 'INSERT INTO rezagoAlumno (idAlumnos, descripcion) VALUES (?, ?)';
    
    // Ejecutar la consulta SQL con los parámetros proporcionados
    await connection.execute(query, [idAlumnos, descripcion]);
